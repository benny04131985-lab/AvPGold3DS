...
#define AVP3DS_SHIP1_PERF_LOG_PATH "sdmc:/AVP_SHIP1_PERF.log"
static bool avp3ds_ship1_perf_log_initialized = false;

/* AVP-HEADROOM2-SUBBUCKET1: diagnostic subsystem timing totals. */
#define AVP3DS_HEADROOM_BUCKET_COUNT 19
...
void AvP3DS_HeadroomAccumulate(
    int bucket,
    unsigned long long startTick)
{
    if (bucket >= 0 && bucket < AVP3DS_HEADROOM_BUCKET_COUNT)
    {
        ...
        avp3ds_headroom_ticks[bucket] +=
            finishTick - (u64)startTick;
        ++avp3ds_headroom_calls[bucket];
    }
}

static double AvP3DS_HeadroomMsPerFrame(
    int bucket,
    double sampleCount)
{
    if (bucket < 0 || bucket >= AVP3DS_HEADROOM_BUCKET_COUNT)
    ...
    return
        ((double)avp3ds_headroom_ticks[bucket] / sampleCount) /
        CPU_TICKS_PER_MSEC;
}

static double AvP3DS_HeadroomCallsPerFrame(
    int bucket,
    double sampleCount)
{
    if (bucket < 0 || bucket >= AVP3DS_HEADROOM_BUCKET_COUNT)
...
static void AvP3DS_WriteHeadroomReport(
    FILE *stream,
    u32 frame,
    double sampleCount)
{
    ...
    fprintf(
        ...
        "U netC=%.2f netS=%.2f sndEnv=%.2f sndMgmt=%.2f plySnd=%.2f\n"
        "P view=%.2f hud=%.2f input=%.2f sound=%.2f menu=%.2f finish=%.2f\n"
        "C netC=%.2f netS=%.2f menu=%.2f\n",
        frame,
...
void AvP3DS_GameFrameEnd(void)
{
    ...
    if ((avp3ds_diag_frame_count % 120U) == 0U)
    {
        ...
        /* AVP-HEADROOM2-SUBBUCKET1: named subsystem report for the same 120-frame window. */
