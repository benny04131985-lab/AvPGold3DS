...
extern unsigned int AvP3DS_ButtonsHeld(void);
extern void AvP3DS_ReadCirclePad(int *x, int *y);
extern void AvP3DS_ReadCStick(int *x, int *y);
extern void AvP3DS_BenchmarkFrameStart(void);
extern void AvP3DS_BenchmarkUpdateStart(void);
extern void AvP3DS_BenchmarkUpdateEnd(void);
extern void AvP3DS_BenchmarkRenderStart(void);
extern void AvP3DS_BenchmarkRenderEnd(void);
/* AVP-HEADROOM2-SUBBUCKET1: diagnostic subsystem timers. */
extern unsigned long long AvP3DS_HeadroomTick(void);
extern void AvP3DS_HeadroomAccumulate(int bucket, unsigned long long startTick);
#endif
#ifdef __3DS__
#undef SDL_INIT_VIDEO
#define SDL_INIT_VIDEO 0

extern int AvP3DS_AppRunning(void);
extern int AvP3DS_StartPressed(void);
#endif
#ifdef __3DS__
#define SDL_SetRelativeMouseMode(mode) ((void)(mode))
extern void AvP_PresentSurface3DS(SDL_Surface *surface);
extern void AvP3DS_GameFrameEnd(void);
extern void AvP3DS_SetStereoFlatFrame(int flat);
#define SDL_GL_SwapWindow(win) AvP_PresentSurface3DS(surface)
#endif
#include "oglfunc.h"
...
/*
 * AvP's legacy software renderer used OpenGL only to display its CPU buffer.
 * The 3DS port presents that buffer directly through libctru instead.
 */
#define pglViewport(...)              ((void)0)
#define pglEnable(...)                ((void)0)
#define pglBlendFunc(...)             ((void)0)
#define pglDepthFunc(...)             ((void)0)
#define pglDepthMask(...)             ((void)0)
#define pglDepthRange(...)            ((void)0)
#define pglDisable(...)               ((void)0)
#define pglClear(...)                 ((void)0)
#define pglTexParameteri(...)         ((void)0)
#define pglGetIntegerv(...)           ((void)0)
#define pglGenTextures(...)           ((void)0)
#define pglGenFramebuffers(...)       ((void)0)
#define pglGenRenderbuffers(...)      ((void)0)
#define pglBindTexture(...)           ((void)0)
#define pglTexImage2D(...)            ((void)0)
#define pglBindFramebuffer(...)       ((void)0)
#define pglFramebufferTexture2D(...)  ((void)0)
#define pglRenderbufferStorage(...)   ((void)0)
#define pglFramebufferRenderbuffer(...) ((void)0)
#define pglGenBuffers(...)            ((void)0)
#define pglCheckFramebufferStatus(...) (0)
#define pglBindRenderbuffer(...)      ((void)0)
#define pglPixelStorei(...)           ((void)0)
#define pglReadPixels(...)            ((void)0)
#define pglTexSubImage2D(...)         ((void)0)
#define pglBindBuffer(...)            ((void)0)
#define pglBufferData(...)            ((void)0)
#define pglVertexAttribPointer(...)   ((void)0)
#define pglDrawElements(...)          ((void)0)

#define check_for_errors_(...)        ((void)0)
#define DrawFullscreenTexture(...)    ((void)0)
#define SelectProgram(...)            ((void)0)
#define InitOpenGL(...)               (0)
...
#ifdef __3DS__
        {
                const int circleDeadZone = 35;
                unsigned int avp3dsButtons;
                int circleX = 0;
                int circleY = 0;

                avp3dsButtons = AvP3DS_ButtonsHeld();
                AvP3DS_ReadCirclePad(&circleX, &circleY);

                /*
                 * YEET34B native 3DS movement:
                 * Circle Pad injects AvP's configured arrow-key movement.
                 * It also remains usable for menu navigation.
                 */
                handle_keypress(
                        KEY_UP,
                        0,
                        circleY > circleDeadZone);
                ...
                /*
                 * YEET34C fixed native D-pad controls.
                 *
                 * Keep these gameplay-only so Backspace does not affect
                 * profile-management or key-configuration menus.
                 */
                    ...
                    !InGameMenusAreRunning())
                {
                        handle_keypress(
                                ...
                                0,
                                (avp3dsButtons &
                                 AVP3DS_BUTTON_RIGHT) != 0);

                        handle_keypress(
                                KEY_LBRACKET,
                                0,
                                (avp3dsButtons &
                                 AVP3DS_BUTTON_LEFT) != 0);
                        ...
                        handle_keypress(
                                ...
                                0,
                                (avp3dsButtons &
                                 AVP3DS_BUTTON_DOWN) != 0);
                }
                else
                {
                        handle_keypress(KEY_RBRACKET, 0, 0);
                        handle_keypress(KEY_LBRACKET, 0, 0);
                        handle_keypress(KEY_BACKSPACE, 0, 0);
                        handle_keypress(KEY_COMMA, 0, 0);
                }

                /*
                 * A operates objects during gameplay and also confirms
                 * Enter-driven menu choices.
                 */
                handle_keypress(
                        KEY_SPACE,
                        0,
                ...
                /*
                 * Shared face-button layout:
                 * B = jump, ZL = crouch, X = jetpack/grapple.
                 */
                handle_keypress(
                        KEY_RIGHTSHIFT,
                        0,
                ...
                /*
                 * New 3DS shoulder controls:
                 * ZR handles the species-specific slash action.
                 * Y handles the former ZL period action.
                 */
                handle_keypress(
                        KEY_SLASH,
                        0,
                ...
                /*
                 * YEET33B ergonomic crouch mapping:
                 * ZL is easy to hold while operating the C-nub,
                 * which makes Alien wall-crawling practical.
                 */
                handle_keypress(
                        KEY_RIGHTCTRL,
                        0,
                ...
                /*
                 * START is AvP's normal Escape/menu/back control.
                 * SELECT remains the development safe-exit button.
                 */
                handle_keypress(
                        KEY_ESCAPE,
                        0,
                        (avp3dsButtons &
                         AVP3DS_BUTTON_START) != 0);
...
#ifdef __3DS__
        /*
         * SDL relative-mouse mode is stubbed on 3DS, but AvP still requires
         * its mouse path for C-stick look and L/R mouse-button mappings.
         */
        wantmouse = 1;
#else
        wantmouse =
...
#ifdef __3DS__
        {
                const int cstickDeadZone = 12;
                const int cstickDivisor = 6;
                int cstickX = 0;
                int cstickY = 0;

                AvP3DS_ReadCStick(&cstickX, &cstickY);

                x = 0;
                y = 0;
                buttons = 0;

                if (cstickX > cstickDeadZone ||
                    cstickX < -cstickDeadZone)
                {
                        x = (cstickX * 5) / (cstickDivisor * 4);
                }

                if (cstickY > cstickDeadZone ||
                    cstickY < -cstickDeadZone)
                {
                        y = -(cstickY * 5) / (cstickDivisor * 4);
                }
        }
#else
...
#ifdef __3DS__
    /*
     * Gameplay was rendered directly to the top-screen Citro3D target.
     * Present that frame without overwriting it with the SDL menu surface.
     */
    AvP3DS_GameFrameEnd();
    return;
#else
...
#ifdef __3DS__
int AvP_LegacyMain(int argc, char* argv[])
#else
...
#endif
{
     ...
     *
     * Stop when the front-end reaches terminal state 0;
     * aptMainLoop() alone remains true until HOME closes
     * the application.
     */
    while (AvP3DS_AppRunning() && main_loop_state != 0)
    {
        if (AvP3DS_StartPressed())
            break;
...
static void main_loop(void) {
	switch (main_loop_state) {
		...
		case -1:
		        /* Front-end deinit completed after EXIT GAME. */
		        main_loop_state = 0;
		        return;
...
static int MainGame_Init(void) {
	#ifdef __3DS__
   /*
    * SetOGLVideoMode() is skipped on 3DS, so initialize the temporary
    * null-OpenGL table before gameplay touches any pgl function.
    */
   load_ogl_functions(0);
#endif
		...
		case I_GM_Playing:
			if ((!menusActive || (AvP.Network!=I_No_Network && !netGameData.skirmishMode)) && !AvP.LevelCompleted) {
...
#ifdef __3DS__
				AvP3DS_BenchmarkUpdateStart();
#endif

				#ifdef __3DS__
				{
				    unsigned long long avp3ds_hr_start = AvP3DS_HeadroomTick();
				    DoAllShapeAnimations();
				    AvP3DS_HeadroomAccumulate(0, avp3ds_hr_start);
				}
#else
				DoAllShapeAnimations();
#endif
...
#ifdef __3DS__
				AvP3DS_BenchmarkUpdateEnd();
				AvP3DS_BenchmarkRenderStart();
#endif
				
				#ifdef __3DS__
				/*
				 * AVP-STEREO-S2C1A2-FLAT-STATES: evaluate the current player state
				 * after simulation and before native rendering.
				 */
				{
				    PLAYER_STATUS *stereoPlayerStatus = NULL;
				    ...
				    AvP3DS_SetStereoFlatFrame(
				        stereoPlayerStatus == NULL ||
				        !stereoPlayerStatus->IsAlive ||
				        InGameMenusAreRunning() ||
				        AvP.LevelCompleted);
				}
				#endif
				...
				#ifdef __3DS__
				{
				    unsigned long long avp3ds_hr_start = AvP3DS_HeadroomTick();
				    AvpShowViews();
				    AvP3DS_HeadroomAccumulate(13, avp3ds_hr_start);
				}
#else
				AvpShowViews();
#endif
				...
				#ifdef __3DS__
				{
				    unsigned long long avp3ds_hr_start = AvP3DS_HeadroomTick();
				    MaintainHUD();
				    AvP3DS_HeadroomAccumulate(14, avp3ds_hr_start);
				}
#else
				MaintainHUD();
#endif
				...
				#ifdef __3DS__
				{
				    unsigned long long avp3ds_hr_start = AvP3DS_HeadroomTick();
				    ReadUserInput();
				    AvP3DS_HeadroomAccumulate(15, avp3ds_hr_start);
				}
#else
				ReadUserInput();
#endif
				...
				#ifdef __3DS__
				{
				    unsigned long long avp3ds_hr_start = AvP3DS_HeadroomTick();
				    SoundSys_Management();
				    AvP3DS_HeadroomAccumulate(16, avp3ds_hr_start);
				}
#else
				SoundSys_Management();
#endif
				...
				#ifdef __3DS__
				/*
				 * AVP-STEREO-S2C1A2-FLAT-STATES: pause/menu/death/statistics fallback.
				 */
				AvP3DS_SetStereoFlatFrame(1);
				#endif

				ThisFramesRenderingHasBegun();
			}

			menusActive = #ifdef __3DS__
			{
			    unsigned long long avp3ds_hr_start = AvP3DS_HeadroomTick();
			    AvP_InGameMenus();
			    AvP3DS_HeadroomAccumulate(17, avp3ds_hr_start);
			}
#else
			AvP_InGameMenus();
#endif
			...
			#ifdef __3DS__
			{
			    ...
			    AvP3DS_HeadroomAccumulate(18, avp3ds_hr_start);
			}
#else
			ThisFramesRenderingHasFinished();
#endif
...
#ifdef __3DS__
/*
 * First staged 3DS engine probe.
 *
 * This deliberately stops before menus, assets, audio, FMV, OpenGL,
 * level loading, and frame presentation.
 */
int AvP_EngineProbe3DS_Full(void)
{
    WantSound = 0;
    WantCDRom = 0;
    WantJoystick = 0;
    ...
    /*
     * Configure the engine's CPU-rendered RGB565 surface without calling
     * SetSoftVideoMode(), because its desktop path reaches pglViewport().
     */
    RenderingMode = RENDERING_MODE_SOFTWARE;
    ScanDrawMode = ScanDrawD3DHardwareRGB;
    GotMouse = 1;
    ...
    ScreenDescriptorBlock.SDB_CentreX = 320;
    ScreenDescriptorBlock.SDB_CentreY = 240;
    ScreenDescriptorBlock.SDB_ProjX = 320;
    ScreenDescriptorBlock.SDB_ProjY = 240;
    ScreenDescriptorBlock.SDB_ClipLeft = 0;
...
#ifdef __3DS__
int AvP_EngineProbe3DS(void)
{
    SDL_Surface *probeSurface;

    /*
     * Do not initialize SDL video.
     * libctru retains ownership of the physical 3DS display.
     */
    if (SDL_Init(0) != 0)
        return 10;

    probeSurface = SDL_CreateRGBSurface(
        0,
        640,
        480,
        16,
    ...
    if (probeSurface->w != 640 ||
        probeSurface->h != 480 ||
        probeSurface->format == NULL ||
        probeSurface->format->BitsPerPixel != 16)
    {
        SDL_FreeSurface(probeSurface);
        SDL_Quit();
        return 30;
    }
...
int AvP_RendererProbe3DS(void)
{
    ...
    /*
     * Configure the original AvP engine for CPU software rendering.
     * The physical 3DS output remains 400x240.
     */
    RenderingMode = RENDERING_MODE_SOFTWARE;
    ScanDrawMode = ScanDrawD3DHardwareRGB;
    ...
    /*
     * AvP initially renders into its traditional 640x480 coordinate space.
     */
    ScreenDescriptorBlock.SDB_Width = 640;
    ScreenDescriptorBlock.SDB_Height = 480;
    ScreenDescriptorBlock.SDB_CentreX = 320;
    ...
    /*
     * Confirm the engine and SDL surface still agree.
     */
    if (surface->pixels == NULL || surface->pitch <= 0)
    {
        SDL_FreeSurface(surface);
        surface = NULL;
        SDL_Quit();
