...
static C2D_SpriteSheet avp3ds_marine_hud_sheet = NULL;
static C2D_Image avp3ds_marine_hud_image;
static bool avp3ds_marine_hud_ready = false;
...
#define AVP3DS_HUD_GROUP_COUNT     4
...
{
    /*
     * AVP-HUD1C2 alignment pass.
     *
     * Tracker: shifted left by half of one 16-pixel diagnostic cell.
     */
    { 74.0f, 58.0f, 158.0f, 100.0f, 1.55f },

    /*
     * Health and armor: shifted left so the live numeric column begins
     * near the first 32-pixel major guide.
     */
    { 2.0f, 160.0f, 88.0f, 76.0f, 1.45f },

    /*
     * Ammunition: shifted left so the live right edge aligns near the
     * first 32-pixel major guide measured from the right edge.
     */
    { 204.0f, 158.0f, 84.0f, 78.0f, 1.00f },

    /*
     * Mission/objective strip: same height and Y position, centered
     * exactly across the 320-pixel lower screen.
     */
    { 53.0f, 4.0f, 214.0f, 42.0f, 1.00f }
};
...
if (avp3ds_marine_hud_sheet != NULL)
{
    C2D_SpriteSheetFree(avp3ds_marine_hud_sheet);
    avp3ds_marine_hud_sheet = NULL;
    memset(
        &avp3ds_marine_hud_image,
        0,
        sizeof(avp3ds_marine_hud_image));
    avp3ds_marine_hud_ready = false;
}
...
memset(
    ...
    0,
    sizeof(avp3ds_marine_hud_image));
avp3ds_marine_hud_sheet =
    C2D_SpriteSheetLoad(
        "romfs:/Marine_WY_HUD.t3x");

if (avp3ds_marine_hud_sheet != NULL)
{
    if (C2D_SpriteSheetCount(
            avp3ds_marine_hud_sheet) >= 1)
    {
        avp3ds_marine_hud_image =
        ...
        if (avp3ds_marine_hud_image.subtex != NULL)
        {
            avp3ds_marine_hud_ready = true;
        }
        else
        {
            printf(
                "Marine HUD backdrop metadata missing.\n");
            C2D_SpriteSheetFree(
                avp3ds_marine_hud_sheet);
            avp3ds_marine_hud_sheet = NULL;
        }
    ...
    {
        printf(
            "Marine HUD backdrop sheet is empty.\n");
        C2D_SpriteSheetFree(
            avp3ds_marine_hud_sheet);
        avp3ds_marine_hud_sheet = NULL;
    }
...
{
    printf(
        "Marine HUD backdrop unavailable; keeping diagnostic grid.\n");
}
 ...
 */
static void AvP3DS_DrawMarineHUDBackdrop(void)
{
    C2D_DrawParams parameters;

    if (!avp3ds_marine_hud_ready ||
        avp3ds_bottom_target == NULL ||
        avp3ds_marine_hud_image.subtex == NULL)
    {
        return;
    }
    ...
    parameters.pos.x = 0.0f;
    parameters.pos.y = 0.0f;
    parameters.pos.w = AVP3DS_BOTTOM_WIDTH;
    parameters.pos.h = AVP3DS_BOTTOM_HEIGHT;
...
if (avp3ds_marine_hud_ready)
{
    ...
    AvP3DS_DrawMarineHUDBackdrop();
    /* AVP-HUD1D3 robust lower-target backdrop flush */
    C2D_Flush();
}
