...
#ifdef __3DS__
extern void AvP3DS_SetLoadingProgress(
    int fixedPosition,
    int visible);
#endif
...
#else
	/*
	 * AVP-HUD1A suppress legacy loading artwork:
	 * the 3DS uses the single native progress strip drawn after presentation.
	 */
#endif
...
#ifdef __3DS__
	/*
	 * AVP-HUD1B1 stable loading bar:
	 * provide the progress value before presenting so the briefing and
	 * bar become one persistent GPU frame.
	 */
	AvP3DS_SetLoadingProgress(
		CurrentPosition,
		LoadingBarFull != NULL);
#endif
