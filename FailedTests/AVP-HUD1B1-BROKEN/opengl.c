 ...
 */
static int avp3ds_triangle_route =
    AVP3DS_DRAW_ROUTE_TOP;
#endif
...
#ifdef __3DS__
void AvP3DS_SetTriangleRoute(int route)
{
    if (route != AVP3DS_DRAW_ROUTE_MIRROR_BOTTOM)
        route = AVP3DS_DRAW_ROUTE_TOP;

    if (route == avp3ds_triangle_route)
        return;

    /*
     * Submit all geometry queued for the old target before changing
     * ownership of subsequent batches.
     */
    FlushTriangleBuffers(1);
    avp3ds_triangle_route = route;
}
#endif
