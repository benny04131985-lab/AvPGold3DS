...
static char *FixFilename(const char *filename, const char *prefix, int force)
{
	...
	/*
	 * Avoid duplicate separators when the supplied prefix already ends
	 * with one. This matters for paths such as "User_Profiles/".
	 */
	if (plen > 0 &&
	    prefix[plen - 1] != '/' &&
	    prefix[plen - 1] != '\\' &&
	...
	f = (char *)malloc(strlen(filename) + flen + 1);
	strcpy(f, prefix);
...
static int GetFA(const char *filename)
{
	...
	if (stat(filename, &buf) == 0) {
		...
		#ifdef __3DS__
		/*
		 * On 3DS, successful stat/open operations are more reliable than
		 * access(R_OK/W_OK) for directory-entry permission reporting.
		 */
