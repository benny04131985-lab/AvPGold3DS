...
extern void AvP3DS_GameFrameBegin(void);
extern void AvP3DS_SetStereoFlatBatch(int flat);
extern void AvP3DS_SetHUDCaptureGroup(int group);

extern void AvP3DS_DestroyNativeTexture(
    void *textureHandle);

extern void AvP3DS_DrawTriangles(
    const void *vertices,
    int vertexCount,
...
#ifdef __3DS__
        /*
         * AvP has already transformed these vertices into clip space.
         * Submit the completed batch directly to Citro3D.
         */
        AvP3DS_DrawTriangles(
            varr,
            varrc,
            tarr,
            tarrc,
            CurrentlyBoundTexture,
            (int)CurrentTranslucencyMode);
#else
        pglBufferData(
            GL_ARRAY_BUFFER,
            varrc * sizeof(varr[0]),
 ...
 * Flush at both boundaries so each capture batch keeps one identity.
 */
/*
 * AVP-STEREO-S2C2-FLAT-SIGHTS: flush-safe stereo overlay boundary.
 */
void AvP3DS_BeginFlatStereoOverlay(void)
{
    FlushTriangleBuffers(1);
    AvP3DS_SetStereoFlatBatch(1);
}

void AvP3DS_EndFlatStereoOverlay(void)
{
    FlushTriangleBuffers(1);
    AvP3DS_SetStereoFlatBatch(0);
}

void AvP3DS_BeginHUDCapture(int group)
{
    FlushTriangleBuffers(1);
    AvP3DS_SetHUDCaptureGroup(group);
}

void AvP3DS_EndHUDCapture(void)
{
    FlushTriangleBuffers(1);
    AvP3DS_SetHUDCaptureGroup(-1);
}
#endif
...
static void CheckBoundTextureIsCorrect(D3DTexture *tex)
{
    ...
    /*
     * Geometry already queued belongs to the previously bound texture.
     * Submit that batch before changing the material pointer.
     */
    FlushTriangleBuffers(1);
...
#ifdef __3DS__
	{
		static GLuint next3DSTextureID = 1;
		GLuint h = next3DSTextureID++;

		/*
		 * Desktop OpenGL does not exist on 3DS. Keep the decoded RGBA
		 * pixels attached to the texture for the future native renderer.
		 */
		tex->buf = buf;
		tex->id = h;
...
void ReleaseD3DTexture(void *tex)
{
	...
	#ifdef __3DS__
        AvP3DS_DestroyNativeTexture(TextureHandle);
#endif
...
void ColourFillBackBuffer(int FillColour)
{
    ...
    {
        Uint8 blue =
            (Uint8)((FillColour >> 0) & 0xFF);

        Uint8 green =
            (Uint8)((FillColour >> 8) & 0xFF);

        Uint8 red =
            (Uint8)((FillColour >> 16) & 0xFF);

        Uint8 alpha =
            (Uint8)((FillColour >> 24) & 0xFF);
