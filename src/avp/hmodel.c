...
void SaveHierarchy(HMODELCONTROLLER* controller)
{
        ...
        const char* Hierarchy_Name;
        const char* Rif_Name;
        unsigned int hierarchyNameLength;
        unsigned int rifNameLength;
        unsigned int blockSize;

        if(!controller) return;
        if(!controller->Root_Section) return;

        Hierarchy_Name = controller->Root_Section->Hierarchy_Name;
        Rif_Name = controller->Root_Section->Rif_Name;

        hierarchyNameLength =
                (unsigned int)strlen(Hierarchy_Name) + 1U;
        rifNameLength =
                (unsigned int)strlen(Rif_Name) + 1U;

        /*
         * The hierarchy structure and both names form one logical save
         * block. Allocate them together so 3DS alignment is applied only
         * after the complete block, not between the two strings.
         */
        blockSize =
        ...
        block = (HIERARCHY_SAVE_BLOCK*)
                GetPointerForSaveBlock(blockSize);

        //fill in header
        block->header.type = SaveBlock_Hierarchy;
        block->header.size = blockSize;
        ...
        block->root_section_id =
                controller->Root_Section->IDnumber;

        //append both names inside the same logical block
        buffer = (char*)(block + 1);

        strcpy(buffer,Hierarchy_Name);
        buffer += hierarchyNameLength;
