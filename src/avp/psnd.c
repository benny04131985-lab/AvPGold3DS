...
/* AVP-HEADROOM3-AUDIOPOLL1: one locked voice-status snapshot per management pass. */
#ifdef __3DS__
extern void AvP3DS_BeginSoundStatusSnapshot(void);
extern void AvP3DS_EndSoundStatusSnapshot(void);
#endif
#ifdef __3DS__
/* AVP-SHIPFINAL1-VOICEHALF-MENUSAFE: active-voice completion/3D attenuation at half frame cadence. */
static unsigned int avp3ds_sound_voice_maintenance_phase = 0;
#endif
...
void SoundSys_Management(void)
{
	...
	int num3dUpdates = 0;
#ifdef __3DS__
	int avp3ds_do_voice_maintenance;
#endif

	if(!SoundSwitchedOn) return;

#ifdef __3DS__
    avp3ds_do_voice_maintenance =
        ((avp3ds_sound_voice_maintenance_phase++ & 1U) == 0U);

    if (avp3ds_do_voice_maintenance)
    {
        AvP3DS_BeginSoundStatusSnapshot();
#endif
