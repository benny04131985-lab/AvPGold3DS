...
#else
static inline void PlayBinkedFMV(const char* filenamePtr, int volume)
{
	(void)filenamePtr;
	(void)volume;
}
...
#ifdef __3DS__
extern int AvP3DS_ShowKeyboard(
    char *buffer,
    int bufferBytes,
    const char *hint);
#endif
...
#ifdef __3DS__
    /*
     * AVP-SHIPFINAL1-VOICEHALF-MENUSAFE: unsupported desktop/network/debug menu routes are not part
     * of the 3DS ship surface. Hidden UI is backed by a hard redirect so a
     * stale profile/menu state cannot enter those paths.
     */
    switch (menuID)
    {
        case AVPMENU_MAIN_WITHCHEATS:
        case AVPMENU_DEBUG_MAIN:
        case AVPMENU_DEBUG_MAIN_WITHCHEATS:
        case AVPMENU_MULTIPLAYER:
        case AVPMENU_MULTIPLAYER_LOBBIEDSERVER:
        case AVPMENU_MULTIPLAYER_LOBBIEDCLIENT:
        case AVPMENU_MULTIPLAYER_SKIRMISH:
        case AVPMENU_MULTIPLAYERSTARTGAME:
        case AVPMENU_MULTIPLAYERJOINGAME:
        case AVPMENU_MULTIPLAYERSELECTSESSION:
        case AVPMENU_MULTIPLAYERJOINGAME2:
        case AVPMENU_MULTIPLAYEROPENADDRESS:
        case AVPMENU_MULTIPLAYER_LOADIPADDRESS:
        ...
        case AVPMENU_MULTIPLAYER_CONFIG_JOIN:
        case AVPMENU_SKIRMISH_CONFIG:
        case AVPMENU_MULTIPLAYER_SPECIES_HOST:
        case AVPMENU_MULTIPLAYER_SPECIES_JOIN:
        case AVPMENU_MULTIPLAYER_SAVECONFIG:
        case AVPMENU_MULTIPLAYER_LOADCONFIG:
        case AVPMENU_MULTIPLAYER_DELETECONFIG:
        case AVPMENU_MULTIPLAYER_JOINING:
...
#else
			/* AVP-SHIPFINAL1-VOICEHALF-MENUSAFE: debug/cheat main-menu variants are not ship UI. */
#endif
			break;
		}
...
#ifdef __3DS__
			if (!AvP3DS_ShowKeyboard(
					UserProfilePtr->Name,
					(int)sizeof(UserProfilePtr->Name),
					"Enter profile name"))
			{
				strcpy(UserProfilePtr->Name, "DeadMeat");
			}

			AvPMenus.PositionInTextField =
				strlen(UserProfilePtr->Name);
...
static void RenderLoadGameMenu(void)
{
	...
	else // in game menus
	{
		y = (ScreenDescriptorBlock.SDB_Height - AvPMenus.MenuHeight)/2;
		RenderText =
			(int (*)(const char*, int, int, int, enum AVPMENUFORMAT_ID))
			Hardware_RenderSmallMenuText;
	}
...
//				netGameData.myGameState = NGS_Playing;
		...
		case AVPMENU_ELEMENT_SAVEGAME:
		{
			...
			{
				//set the request
				/* AVP-SHIPFINAL1-VOICEHALF-MENUSAFE: fixed-name save trace removed for ship. */
