...
#include <string.h>
/* YEET30 ARM-safe ObjectID copies */
#include <stdlib.h>
		...
		for(int i=0;i<ts->num_point_effects;i++)
		{
			...
			if(stp->num_targets)
			{
				...
				for(int j=0;j<stp->num_targets;j++)
				{
					...
					memcpy(
					    &stp->targets[j].target_name[0],
					    &tpe->targets[j].targetID,
					    sizeof(ObjectID));
				}
...
static void add_message_strategy(const char* name,AVP_Strategy_Chunk* asc,ObjectID ID)
{
	...
	/*
         * nameID may not be aligned for ObjectID on ARM.
         */
        memcpy(&mtt->nameID[0], &ID, sizeof ID);
...
void setup_particle_generators(Environment_Data_Chunk * envd)
{
	...
	for(LIF<Chunk*> chlif(&chlist);!chlif.done();chlif.next())
	{
		...
		memcpy(
		    ...
		    sizeof(ObjectID));
		memcpy(
		    &part_temp->parentID[0],
		    &data_chunk->parent_id,
		    sizeof(ObjectID));
		...
		if(data_chunk->type==PARGEN_TYPE_SPARK)
			...
			part_temp->frequency=
				ONE_FIXED/(std::max)(data_chunk->quantity, (unsigned short)1);
