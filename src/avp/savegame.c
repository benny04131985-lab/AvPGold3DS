...
static unsigned int GetStoredSaveBlockSize(unsigned int size)
{
#ifdef __3DS__
        return (size + 3U) & ~3U;
#else
...
unsigned int storedBlockSize = GetStoredSaveBlockSize(size);
	
	//see if we need to enlarge the buffer to allow for this block
	if(storedBlockSize > SaveInfo.BufferSpaceLeft)
	{
		//need more space
		int spaceNeeded = SaveInfo.BufferSpaceUsed + storedBlockSize;
	...
	retPointer = (void*)SaveInfo.BufferPos;
        ...
        if(storedBlockSize > size)
        {
                unsigned int paddingIndex;

                for(
                        paddingIndex = size;
                        paddingIndex < storedBlockSize;
                        paddingIndex++)
                {
                        ((char*)retPointer)[paddingIndex] = 0;
                }
        }

	SaveInfo.BufferSpaceUsed += storedBlockSize;
	SaveInfo.BufferSpaceLeft -= storedBlockSize;
	SaveInfo.BufferPos += storedBlockSize;
...
void LoadSavedGame()
{
 ...
 int terminal_error_stage = 0;
 unsigned int terminal_error_type = 0;
 unsigned int terminal_error_size = 0;
 unsigned int terminal_error_left = 0;
#ifdef __3DS__
 unsigned char avp3dsLoadErrorMessage[96];
#endif
	...
	// attempt to access level header	
	{
		...
		if(header->size == 0 || GetStoredSaveBlockSize(header->size) > LoadInfo.BufferSpaceLeft)
		{
			//oh dear, dodgy file
			GLOBALASSERT("Invalid save game header size"==0);
			terminal_error = TRUE;
			terminal_error_stage = 1;
			terminal_error_type = (unsigned int)header->type;
			terminal_error_size = header->size;
			terminal_error_left = LoadInfo.BufferSpaceLeft;
		}
		else
		{
			//go to the next header
			LoadInfo.BufferPos += GetStoredSaveBlockSize(header->size);
			LoadInfo.BufferSpaceLeft -= GetStoredSaveBlockSize(header->size);
...
#ifdef __3DS__
		sprintf(
		        (char*)avp3dsLoadErrorMessage,
		        "LOAD ERR S%d T%u Z%u L%u",
		        terminal_error_stage,
		        terminal_error_type,
		        terminal_error_size,
		        terminal_error_left);
		NewOnScreenMessage(avp3dsLoadErrorMessage);
#else
		NewOnScreenMessage(
		        GetTextString(
